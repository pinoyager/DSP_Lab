...
matrix_back = [cos(radius * -1) -sin(radius * -1) ; sin(radius * -1) cos(radius * -1)];
...
% from the source image, and save to R_rot, G_rot, B_rot.
 for y_new = 1 : height_new
     for x_new = 1 : width_new
        ...
        old_value = [x_new - x_shift ,y_new - y_shift] * matrix_back;
                
        % step6. using "ceil()" & "floor()" to get interpolation coordinates
        % x1, x2, y1, y2
        x1 = floor(old_value(1));
        x2 = ceil(old_value(1));
        y1 = floor(old_value(2));
        y2 = ceil(old_value(2));
        ...
        if (x1 >= 1) && (x1 <= width) && (x2 >= 1) && (x2 <= width) && ...
            ...
            if(x1 == x2 || y1 == y2)
                ...
                wa = (old_value(1) - x1)/(x2 - x1);
                wb = (old_value(2) - y1)/(y2 - y1);
            end
